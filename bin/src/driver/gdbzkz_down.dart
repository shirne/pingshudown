...
class GdbzkzDown extends DownDriver {
  static const baseUrl = 'http://www.gdbzkz.com/';
  ...
  Future<void> download(int idx) async {
    ...
    final characterTitle = dom.querySelector('.content h1')?.text ?? '';
    final content = dom.querySelector('.content .showtxt')?.text ?? '';
  ...
  Future<void> start({int startId = 0}) async {
    ...
    final dom = parse(html);
    final infoBox = dom.querySelector('.book .info');
    if (infoBox == null) {
      stdout.writeln('Parse info error');
      return;
    }
    title = infoBox.querySelector('h2')?.text ??
        dom.head?.querySelector('title')?.text ??
        '';
    stdout.writeln('title: $title');
    final spans = infoBox.querySelectorAll('.small span');
    ...
    final links = dom.querySelector('.listmain dl')?.children;
    if (links == null) {
    ...
    for (Element e in links) {
      ...
      } else if (e.localName == 'dd' && start) {
        final url = e.firstChild?.attributes['href'];
        if (url != null) {
          urls.add(url);
        }
      }
